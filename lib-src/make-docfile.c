...
/* The arguments given to this program are all the C and some Lisp
 source files of GNU Emacs.  .el and .c files are allowed. A .o file
 can also be specified; the .c file it was made from is used. This
 helps the makefile pass the correct list of files. Option -d DIR
 means change to DIR before looking for files.

 The results, which go to standard output or to a file
 specified with -a or -o (-a to append, -o to start from nothing),
 are entries containing function or variable names and their
 documentation. Each entry starts with a ^_ character. Then comes F
 for a function or V for a variable. Then comes the function or
 variable name, terminated with a newline. Then comes the
 documentation for that function or variable.
 */
...
/* Defined to be sys_chdir in ms-w32.h, but only #ifdef emacs, so this
   ...
   Similarly, msdos defines this as sys_chdir, but we're not linking
   with the file where that function is defined.  */
#undef chdir
#endif /* not DOS_NT */
...
struct rcsoc_state
{
  /* A count of spaces and newlines that have been read, but not
   * output.  */
...
{
  if (state->keyword && *state->cur_keyword_ptr == ch
...
/* Skip a C string or C-style comment from INFILE, and return the
   ...
   If SAW_USAGE is non-null, then any occurrences of the string
   "usage:" at the beginning of a line will be removed, and *SAW_USAGE
   set to true if any were encountered.  */
...
/* Write to stdout the argument names of function FUNC, whose text is
   in BUF. MINARGS and MAXARGS are the minimum and maximum number of
   arguments.  */
      ...
      if (c == ',' || c == ')')
        {
          if (ident_length == 0)
            {
              error ("empty arg list for '%s' should be (void), not "
                     "()",
                     func);
              continue;
            }
...
/* The types of globals.  These are sorted roughly in decreasing
   alignment order to avoid allocation gaps, except that symbols and
   functions are last.  */
enum global_type
{
  INVALID,
...
/* Bit values for FLAGS field from the above.  Applied for DEFUNs
 * only.  */
enum
{
  DEFUN_noreturn = 1,
  ...
  if (strcmp (name, "..."))
    {
      if (num_globals == num_globals_allocated)
        {
          ptrdiff_t num_globals_max
            = (min (PTRDIFF_MAX, SIZE_MAX) / sizeof *globals);
          ...
          globals = xrealloc (globals, num_globals_allocated
                                         * sizeof *globals);
        }
      ...
      char *buf
        = xmalloc (namesize + (svalue ? strlen (svalue) + 1 : 0));
      globals[num_globals - 1].type = type;
      globals[num_globals - 1].name = strcpy (buf, name);
      if (svalue)
        globals[num_globals - 1].v.svalue
          = strcpy (buf + namesize, svalue);
      else
...
compare_globals (const void *a, const void *b)
{
  ...
  /* Consider "nil" to be the least, so that iQnil is zero.  That
     way, Qnil's internal representation is zero, which is a bit
     faster. Similarly, consider "t" to be the second-least, and so
     forth.  */
  if (ga->type == SYMBOL)
    {
      ...
      for (int i = 0; i < ncommonsym; i++)
        {
          if (ga->name[0] == 'Q'
              && strcmp (ga->name + 1, commonsym[i]) == 0)
            ai = i;
          if (gb->name[0] == 'Q'
              && strcmp (gb->name + 1, commonsym[i]) == 0)
            bi = i;
        }
...
write_globals (void)
{
  ...
  qsort (globals, num_globals, sizeof (struct global),
         compare_globals);
      ...
      while (i + 1 < num_globals
             && strcmp (globals[i].name, globals[i + 1].name) == 0)
        {
          if (globals[i].type == FUNCTION
            ...
            error ("function '%s' defined twice with differing "
                   "signatures",
                   globals[i].name);
          free (globals[i].name);
          i++;
  ...
  if (infile == NULL && extension == 'o')
    {
      ...
      if (infile == NULL)
        filename[strlen (filename) - 1]
          = 'c'; /* Don't confuse people.  */
    }

  if (infile == NULL)
    {
      char *newfilename = malloc (strlen (filename));
      strcpy (newfilename, filename);
      newfilename[strlen (filename)] = 'm';
      newfilename[strlen (filename) - 1] = 'm';
      infile = fopen (newfilename, mode);
      if (infile != NULL)
        return;
      else
        {
          newfilename[strlen (filename)] = 'c';
          newfilename[strlen (filename) - 1] = 'c';
          infile = fopen (newfilename, mode);
        }
    }
  ...
  while (!feof (infile))
    {
      ...
      if (generate_globals)
        {
          ...
          /* The following code tries to recognize function
             attributes specified after the docstring, e.g.:
      ...
      if (c == '"'
              ...
              && (c = getc (infile), ungetc (c, infile), c == '*')))
        {
          ...
          /* If this is a defun, find the arguments and print them.
             If this function takes MANY or UNEVALLED args, then the
             C source won't give the names of the arguments, so we
             shouldn't bother trying to find them.

             Various doc-string styles:
              0: DEFUN (..., "DOC") (args)            [!comment]
              1: DEFUN (..., /\* DOC *\/ (args))      [comment &&
             !doc_keyword] 2: DEFUN (..., doc: /\* DOC *\/) (args)
             [comment && doc_keyword]
          */
...
/* Read a file of Lisp source code.
 ...
 and we use that instead of reading a doc string within that
 defining-form.

 For defvar, defconst, and fset we skip to the docstring with a
 kludgy formatting convention: all docstrings must appear on the
 same line as the initial open-paren (the one in column zero) and
 must contain a backslash and a newline immediately after the
 initial double-quote. No newlines must appear between the beginning
 of the form and the first double-quote. For defun, defmacro, and
 autoload, we know how to skip over the arglist, but the doc string
 must still have a backslash and newline immediately after the
 double quote. The only source files that follow this convention are
 autoload-generated files like loaddefs.el; The NAME and DOCSTRING
 are output. NAME is preceded by `F' for a function or `V' for a
 variable. An entry is output only if DOCSTRING has \ newline just
 after the opening ".
 */
  ...
  while (!feof (infile))
    {
      ...
      if (!strcmp (buffer, "defun") || !strcmp (buffer, "defmacro")
          || !strcmp (buffer, "defsubst"))
        {
          ...
          /* If the next three characters aren't `dquote bslash
             newline' then we're not reading a docstring.
           */
          if ((c = getc (infile)) != '"'
              || (c = getc (infile)) != '\\'
              || ((c = getc (infile)) != '\n' && c != '\r'))
          ...
          if (c == '\'')
          ...
          else
            {
              ...
              if (c != ')')
                {
                  fprintf (stderr,
                           "## unparsable quoted name in fset in "
                           "%s\n",
                           filename);
                  continue;
                }
          ...
          if (c == '\'')
          ...
          else
            {
              ...
              if (c != ')')
                {
                  fprintf (stderr,
                           "## unparsable quoted name in autoload "
                           "in "
                           "%s\n",
                           filename);
                  continue;
      ...
      /* At this point, we should gobble a doc string from the input
         file. The opening quote (and leading backslash-newline)
