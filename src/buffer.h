...
#define ZE (current_buffer->text->z)
#define ZE_BYTE (current_buffer->text->z_byte)
...
#define GPT_ADDR                                              \
  (current_buffer->text->beg + current_buffer->text->gpt_byte \
   - BEG_BYTE)

/* Address of end of gap in buffer.  */
#define GAP_END_ADDR                                          \
  (current_buffer->text->beg + current_buffer->text->gpt_byte \
   + current_buffer->text->gap_size - BEG_BYTE)
...
#define Z_ADDR                                                \
  (current_buffer->text->beg + current_buffer->text->gap_size \
...
#define BUF_UNCHANGED_MODIFIED(buf) ((buf)->text->unchanged_modified)
...
#define UNCHANGED_MODIFIED BUF_UNCHANGED_MODIFIED (current_buffer)
...
extern void temp_set_point_both (struct buffer *, ptrdiff_t,
                                 ptrdiff_t);
...
/* Maximum number of bytes in a buffer.
   ...
   working. The ptrdiff_t cast ensures that this is signed, not
   unsigned.  */
...
/* For those very rare cases where you may have a "random" pointer
   into the middle of a multibyte char, this moves to the next
   boundary.  */
...
struct buffer_text
{
  ...
  modiff_count chars_modiff; /* This is modified with character change
                                ...
                                otherwise changed.  */
  modiff_count
    save_modiff; /* Previous value of modiff, as of last
                    time buffer visited or saved a file.  */

  modiff_count
    overlay_modiff; /* Counts modifications to overlays.  */
...
enum
{
  NONEXISTENT_MODTIME_NSECS = -1
};
enum
{
  UNKNOWN_MODTIME_NSECS = -2
};
...
struct buffer
{
  ...
  /* Non-nil means to use word wrapping when displaying continuation
   * lines.  */
  ...
  /* t if "self-insertion" should overwrite; `binary' if it should
     also overwrite newlines and tabs - for editing executables and
     the like.  */
  ...
  /* True if the newline position cache, width run cache and BIDI
     paragraph cache are enabled.  See search.c, indent.c and bidi.c
     for details.  */
  ...
  /* Width, height and types of scroll bar areas for windows
  ...
  /* Time stamp updated each time this buffer is displayed in a
   * window.  */
  ...
  /* This points to the `struct buffer_text' that used for this
     ...
     In an indirect buffer, this is the own_text field of another
     buffer.  */
  ...
  /* Size of the file when modtime was set.  This is used to detect
     the case where the file grew while we were reading it, so the
     modtime is still the same (since it's rounded up to seconds) but
     we're actually not up-to-date.  -1 means the size is unknown.
     Only meaningful if modtime is actually set.  */
  off_t modtime_size;
  ...
  /* Non-zero means disable redisplay optimizations when rebuilding
     the glyph matrices (but not when redrawing).  */
...
/* BUFFER_CEILING_OF (resp. BUFFER_FLOOR_OF), when applied to n,
   return the max (resp. min) p such that
...
/* The BUF_BEGV[_BYTE], BUF_ZV[_BYTE], and BUF_PT[_BYTE] functions
   cannot be used for assignment; use SET_BUF_* functions below for
   that.  */
...
{
  return buf->text->beg + buf->text->gap_size + buf->text->z_byte
         - BEG_BYTE;
}
...
{
  return buf->text->beg + buf->text->gpt_byte + buf->text->gap_size
         - BEG_BYTE;
}
...
INLINE void
BUF_COMPUTE_UNCHANGED (struct buffer *buf, ptrdiff_t start,
                       ptrdiff_t end)
{
...
INLINE void
SET_BUF_BEGV_BOTH (struct buffer *buf, ptrdiff_t charpos,
                   ptrdiff_t byte)
{
  buf->begv = charpos;
  buf->begv_byte = byte;
...
INLINE void
SET_BUF_ZV_BOTH (struct buffer *buf, ptrdiff_t charpos,
                 ptrdiff_t byte)
{
  buf->zv = charpos;
  buf->zv_byte = byte;
...
INLINE void
SET_BUF_PT_BOTH (struct buffer *buf, ptrdiff_t charpos,
                 ptrdiff_t byte)
{
  buf->pt = charpos;
  buf->pt_byte = byte;
...
CHAR_POS_ADDR (ptrdiff_t n)
{
  return ((n < GPT ? 0 : GAP_SIZE)
          + buf_charpos_to_bytepos (current_buffer, n) + BEG_ADDR
          - BEG_BYTE);
}
...
/* Convert PTR, the address of a byte in the buffer, into a byte
 * position.  */
...
PTR_BYTE_POS (unsigned char const *ptr)
{
  ...
  return byte - (byte <= GPT_BYTE - BEG_BYTE ? 0 : GAP_SIZE)
         + BEG_BYTE;
}
...
/* Initialize the pseudovector header of buffer object.
   BUFFER_LISP_SIZE is required for GC, but BUFFER_REST_SIZE is set up
   just to be consistent with other pseudovectors.  */

INLINE void
BUFFER_PVEC_INIT (struct buffer *b)
{
  XSETPVECTYPESIZE (b, PVEC_BUFFER, BUFFER_LISP_SIZE,
                    BUFFER_REST_SIZE);
}
...
/* This structure holds the default values of the buffer-local
   variables that have special slots in each buffer. The default value
   occupies the same slot in this structure as an individual buffer's
   value occupies in that buffer. Setting the default value also goes
   through the alist of buffers
...
/* This structure marks which slots in a buffer have corresponding
   ...
   be a Lisp-level local variable for the slot, it has no default
   value, and the corresponding slot in buffer_defaults is not used.
 */
...
extern ptrdiff_t overlays_at (EMACS_INT, bool, Lisp_Object **,
                              ...
                              bool);
extern ptrdiff_t sort_overlays (Lisp_Object *, ptrdiff_t,
                                struct window *);
extern void recenter_overlay_lists (struct buffer *, ptrdiff_t);
extern ptrdiff_t overlay_strings (ptrdiff_t, struct window *,
                                  unsigned char **);
...
extern void fix_overlays_before (struct buffer *, ptrdiff_t,
                                 ptrdiff_t);
extern void mmap_set_vars (bool);
extern void restore_buffer (Lisp_Object);
extern void set_buffer_if_live (Lisp_Object);

/* Return B as a struct buffer pointer, defaulting to the current
 * buffer.  */
    ...
    {                                                               \
      ...
      if ((noverlays) > maxlen)                                     \
        {                                                           \
          maxlen = noverlays;                                       \
          SAFE_NALLOCA (overlays, 1, maxlen);                       \
          (noverlays) = overlays_at (posn, false, &(overlays),      \
                                     &maxlen, nextp, NULL, chrq);   \
        }                                                           \
    }                                                               \
  while (false)
...
{
  return current_buffer->overlays_before
...
/* Return character code of multi-byte form at byte position POS.  If
   POS doesn't point the head of valid multi-byte form, only the byte
   at POS is returned.  No range checking.  */
...
/* Return character code of multi-byte form at byte position POS in
   BUF. If POS doesn't point the head of valid multi-byte form, only
   the byte at POS is returned.  No range checking.  */

INLINE int
BUF_FETCH_MULTIBYTE_CHAR (struct buffer *buf, ptrdiff_t pos)
{
  unsigned char *p
    = ((pos >= BUF_GPT_BYTE (buf) ? BUF_GAP_SIZE (buf) : 0) + pos
       + BUF_BEG_ADDR (buf) - BEG_BYTE);
  return STRING_CHAR (p);
}
...
{
  return (buf->text->beg + buf_charpos_to_bytepos (buf, pos)
          - BEG_BYTE
...
BUF_PTR_BYTE_POS (struct buffer *buf, unsigned char *ptr)
{
  ...
  return (
    byte
    - (byte <= BUF_GPT_BYTE (buf) - BEG_BYTE ? 0 : BUF_GAP_SIZE (buf))
    + BEG_BYTE);
}
...
#define PER_BUFFER_VAR_OFFSET(VAR) offsetof (struct buffer, VAR##_)

/* Used to iterate over normal Lisp_Object fields of struct buffer
   (all Lisp_Objects except undo_list).  If you add, remove, or
   reorder Lisp_Objects in a struct buffer, make sure that this is
   still correct.  */

#define FOR_EACH_PER_BUFFER_OBJECT_AT(offset)                     \
  for (offset = PER_BUFFER_VAR_OFFSET (name);                     \
       offset                                                     \
       <= PER_BUFFER_VAR_OFFSET (cursor_in_non_selected_windows); \
       offset += word_size)
...
{
  return XFIXNUM (
    *(Lisp_Object *) (offset + (char *) &buffer_local_flags));
}
...
{
  return (FIXNUMP (width) && 0 < XFIXNUM (width)
              && XFIXNUM (width) <= 1000
            ? XFIXNUM (width)
            : 8);
}
...
/* Return the width of character C.  The width is measured by how many
   ...
   buffer.  The name CHARACTER_WIDTH avoids a collision with
   <limits.h> CHAR_WIDTH.  */
...
CHARACTER_WIDTH (int c)
{
  return (0x20 <= c && c < 0x7f ? 1
          ...
          : c == '\n'           ? 0
          : !NILP (BVAR (current_buffer, ctl_arrow)) ? 2
                                                     : 4);
}

/* Like fetch_string_char_advance, but fetch character from the
   current buffer.  */
...
/* Like fetch_char_advance, but assumes the current buffer is
 * multibyte.  */
