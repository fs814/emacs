...
// 缓存区索引值 共享与 shader 和 C 代码 为了确保Metal
// Shader缓存区索引能够匹配 Metal API Buffer 设置的集合调用
...
//顶点着色函数
vertex RasterizerData vertexShader(uint vertexID [[vertex_id]], constant LcVertex * vertices [[buffer(LcVertexInputIndexVertices)]], constant vector_uint2 *viewportSizePointer [[buffer(LcVertexInputIndexViewportSize)]]) {
